...
def factorize(values, mask=None, reference_values=None, freeze_reference=True):
    ...
    if sum(len(vec) for vec in all_flat_values) == 0:
        ...
        if reference_values is None:
            unique_values, relative_values = torch.unique(torch.cat(all_flat_values).unsqueeze(0), sorted=False, return_inverse=True)
        elif freeze_reference:
            relative_values, unique_values = torch.unique(torch.cat((reference_values, *all_flat_values)).unsqueeze(0), sorted=False, return_inverse=True)[1], reference_values
        else:
            unique_values, relative_values = torch.unique(torch.cat((reference_values, *all_flat_values)).unsqueeze(0), sorted=False, return_inverse=True)
